...
function Subscription(feed, sub) {
  ...
  util.log(util.format(
    'Loading a subscription for %s', self.Subscription.feed
  ));
  client.query(
  ...
  client.query(
    ...
    function onUpdate(err) {
      ...
      if (self.Subscription.subscribers.length === saved) {
        util.log(util.format(
          'Saved subscription for %s.', self.Subscription.feed
        ));
    ...
    function onSubDelete(subErr) {
      ...
      client.query(
        ...
        function onDelete(err) {
          ...
          util.log(util.format(
            'Removed subscription for %s.', self.Subscription.feed
          ));
...
Subscription.prototype.addSubscriber = function(sub) {
  ...
  client.query(
    ...
    function onSubUpdate(err) {
      ...
      util.log(util.format(
        'Added/updated subscriber for %s.', self.Subscription.feed
      ));
    ...
    if (self.Subscription.subscribers[x].callback === callback) {
      ...
      client.query(
        ...
        function onDelete(err) {
          ...
          util.log(util.format(
            'Removed subscriber for %s.', self.Subscription.feed
          ));
