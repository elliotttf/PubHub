        ...
        self.mysql.query(
          ...
          function onSubscribersLoaded(err, subResults, subFields) {
            ...
            for (var i = 0; i < results.length; i++) {
              self.Subscription.subscribers[i] = results[i];
            }
...
Subscription.prototype.save = function() {
  ...
  self.mysql.query(
    'INSERT INTO subscribers ' +
      '(feed, changed, data, contentType, publish) ' +
      'VALUES ' +
      "(?, ?, ?, ?, ?) " +
      'ON DUPLICATE KEY UPDATE ' +
      "changed = ?, data = ?, contentType = ?, publish = ?",
    [
      self.Subscription.feed,
    ...
    function onUpdate(err) {
      ...
      for (var i = 0; i < self.Subscription.subscribers.length; i++) {
        self.mysql.query(
          'INSERT INTO subscribers ' +
...
Subscription.prototype.removeSubscriber = function(callback) {
  ...
  for (var x in self.Subscription.subscribers) {
    if (self.Subscription.subscribers[x].callback === callback) {
      ...
      if (x === 0) {
        self.Subscription.subscribers.splice(x, x + 1);
      }
      else {
        self.Subscription.subscribers.splice(x, x);
      }
      self.mysql.query(
