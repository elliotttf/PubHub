 ...
 * @type {int}
 */
PubHub.REFRESH = 60000; // TODO - make this variable?
...
PubHub.prototype.publish = function(data) {
  ...
  self.Subscription.loadSubscribers();
  self.Subscription.once('subscribersLoaded', function subscribersLoaded(msg) {
    ...
    self.on('publishedOne', function publishedOne(msg) {
      ...
      if (published == self.Subscription.Subscription.subscribers.length) {
        ...
        self.Subscription.unloadSubscribers();
        published = null;
      }
    });
  });
...
PubHub.prototype.supportsPush = function() {
  ...
  method.get(options, function onReq(res) {
    var data = '';
    res.on('data', function onData(chunk) {
      data += chunk;
    });
