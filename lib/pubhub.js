...
function PubHub(subscription) {
  ...
  self.feed = url.parse(subscription.Subscription.feed);
  self.data = subscription.Subscription.data;
  self.subscribers = [];

  // Convert the strings into url objects.
  for (var x in self.Subscription.Subscription.subscribers) {
    if (typeof self.Subscription.Subscription.subscribers[x].callback !== 'undefined') {
      self.subscribers.push(url.parse(self.Subscription.Subscription.subscribers[x].callback));
    }
  }
  ...
  self.Subscription.on('saved', function onSaved(message) {
    // Update our info.
    self.feed = url.parse(subscription.Subscription.feed);
    self.data = subscription.Subscription.data;
    self.subscribers = self.Subscription.Subscription.subscribers;

    // Convert the strings into url objects.
    for (x in self.subscribers) {
      if (typeof self.subscribers[x].callback !== 'undefined') {
