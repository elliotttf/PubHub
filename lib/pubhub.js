...
PubHub.prototype.fetch = function(options) {
  ...
  if (self.getData() !== '') {
    var changed = new Date(self.getChanged());
    options.headers['If-Modified-Since'] = changed.toUTCString();
    if (self.etag !== null) {
  ...
  var req = method.get(options, function onReq(res) {
    ...
    res.on('end', function onEnd() {
      ...
      if (res.statusCode == 301 || res.statusCode == 302 || res.statusCode == 307) {
        ...
        // Send 'nice' headers if we already have data.
        if (self.getData() !== '') {
          redirectOptions.headers['If-Modified-Since'] = changed.toUTCString();
          if (self.etag !== null) {
