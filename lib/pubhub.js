...
function PubHub(subscription, refresh) {
  ...
  self.Subscription.on('error', function onError(err) {
    ...
    if (typeof err === 'object') {
      util.log(util.format('%s: %s', err.name, err.message));

      if (typeof err.errors === 'object') {
        for (var x in err.errors) {
          util.log(util.format('Error on %s: %s', x, err.errors[x]));
        }
      }
    }
  });
...
PubHub.prototype.fetch = function(options) {
  ...
  if (self.getData() !== '' && self.lastModified !== null) {
    options.headers['If-Modified-Since'] = self.lastModified;
  }
  ...
  var req = method.get(options, function onReq(res) {
    ...
    res.on('end', function onEnd() {
      ...
      if (res.statusCode == 301 || res.statusCode == 302 || res.statusCode == 307) {
        if (typeof res.headers.location === 'undefined') {
          util.log(util.format(
            'Missing location header for %s redirect, bailing.', feedString
          ));
          return;
        }
        util.log(util.format(
          'Feed %s redirected to %s', feedString, res.headers.location
        ));
        ...
        // Send 'nice' headers if we already have data.
        if (self.getData() !== '' && self.lastModified !== null) {
          redirectOptions.headers['If-Modified-Since'] = self.lastModified;
        }
      ...
      // See if the server responded with an last-modified and store it.
      ...
      if (dataHash != self.getData()) {
        util.log(util.format(
          'New data found on %s.', feedString
        ));
  ...
  req.on('error', function onError(err) {
    util.log(util.format(
      'There was a problem fetching new content for %s.', feedString
    ));
    util.log(err);
  });
};
...
PubHub.prototype.publishOne = function(subscriber, data, options, retryCount) {
  ...
  util.log(util.format(
    'Publishing %s to %s.', feedString, subscriberString
  ));
  ...
  var req = http.request(options, function onReq(res) {
    res.on('end', function onEnd() {
      if (res.statusCode !== 200 && retryCount < PubHub.RETRY_COUNT) {
        ...
        util.log(util.format(
          'Published %s to %s.', feedString, subscriberString
        ));
  ...
  req.on('error', function onError(err) {
    util.log(util.format(
      'There was a problem publishing new content to %s.',
      subscriberString
    ));
    util.log(err);
    retryCount++;
...
PubHub.prototype.supportsPush = function() {
  ...
  req.on('error', function onSupportsPushError(err) {
    util.log('There was a problem determining if this subscription supports push.');
    util.log(err);
  });
};
